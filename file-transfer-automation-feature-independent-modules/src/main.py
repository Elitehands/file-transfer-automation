...
from src.transfer import read_excel_batches, process_all_batches , copy_excel_status_log
...
def run_transfer_workflow(config: dict, test_mode: bool = False) -> bool:
    ...
    try:
        ...
        copied_excel = copy_excel_status_log(paths["excel_file"], paths["local_gdrive"])
        if not copied_excel:
            logger.error("Could not copy Excel status log to local drive")
            return False
