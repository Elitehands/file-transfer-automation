...
class ConfigManager:
        ...
        try:
            config = {
                ...
                'filter_criteria': {
                    ...
                    'release_status_column': self._get_required_env('RELEASE_STATUS_COLUMN')
                },
                'notifications': {
                ...
                'system': {
                    ...
                    'transfer_timeout': int(os.getenv('TRANSFER_TIMEOUT_SECONDS', '300')),
    ...
    def _validate_paths(self, config: Dict[str, Any]) -> None:
        ...
        if warnings:
            self.logger.warning("Some paths not accessible - may be expected if VPN not connected yet")