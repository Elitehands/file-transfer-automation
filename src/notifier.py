...
This is an automated message from BBU File Transfer System.
    ...
    def _format_completion_message(self, results: Dict[str, Any]) -> str:
        ...
        success_rate = (results['successful_transfers'] / results['total_batches'] * 100) if results['total_batches'] > 0 else 0
...
"""
        ...
        if results['successful_transfers'] > 0:
            ...
            for batch in results['batch_details']:
                if batch['success']:
                    message += f"  - {batch['batch_id']}: {batch['files_copied']} files\n"
        
        if results['failed_transfers'] > 0:
            message += "\n❌ Failed Batches:\n"
            for batch in results['batch_details']:
                if not batch['success']:
                    message += f"  - {batch['batch_id']}: {', '.join(batch['errors'])}\n"
    ...
    def _send_email(self, subject: str, message: str):
        ...
        if 'email' not in self.config:
            self.logger.warning("Email configuration not found, skipping email notification")
            return
